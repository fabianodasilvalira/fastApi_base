...
from app.routers import parecer as parecer_router # Novo router de Parecer
from app.routers import sistemas_autorizados as sistemas_autorizados_router # Novo router de Sistemas Autorizados
...
async def startup_event():
    ...
    print("Tabelas verificadas/criadas no banco de dados (SQLite). Startup completo.")

# Incluir os routers existentes
app.include_router(ocorrencia_router.router, prefix="/api/v1/ocorrencias", tags=["Ocorrências"])
app.include_router(users_router.router, prefix="/api/v1/users", tags=["User"])
