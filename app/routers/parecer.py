...
@router.post(
    ...
    responses={
        ...
        status.HTTP_409_CONFLICT: {"description": "Conflito de dados (ex: referência a ID de ocorrência/usuário inexistente)."},
...
async def create_parecer_endpoint(
...
async def read_pareceres_endpoint(
...
async def read_parecer_endpoint(
    ...
    try:
        ...
        if db_parecer is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Parecer com ID {parecer_id} não encontrado.")
        return db_parecer
    except SQLAlchemyError as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.put(
    ...
    responses={
        ...
        status.HTTP_401_UNAUTHORIZED: {"description": "Não autorizado."},
        status.HTTP_403_FORBIDDEN: {"description": "Proibido."},
        status.HTTP_404_NOT_FOUND: {"description": "Parecer não encontrado para atualização ou recurso referenciado (ocorrência/usuário) não existe."},
...
async def update_parecer_endpoint(
    ...
    try:
        db_parecer = await parecer_service.get_parecer_by_id(db, parecer_id)
        if db_parecer is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Parecer com ID {parecer_id} não encontrado para atualização.")
        ...
        if parecer_update_data.ocorrencia_id is not None and parecer_update_data.ocorrencia_id != db_parecer.ocorrencia_id:
            ...
            if not ocorrencia_check:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Ocorrência com ID {parecer_update_data.ocorrencia_id} referenciada na atualização do parecer não foi encontrada.")
        ...
        if parecer_update_data.user_id is not None and parecer_update_data.user_id != db_parecer.user_id:
            ...
            if not user_check:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Usuário com ID {parecer_update_data.user_id} referenciado na atualização do parecer não foi encontrado.")

        return await parecer_service.update_parecer(db, db_parecer, parecer_update_data)
    except IntegrityError as e:
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Não foi possível atualizar o parecer {parecer_id} devido a um conflito de dados. Verifique as referências. Erro: {str(e.orig)}"
        )
    except SQLAlchemyError as e:
        await db.rollback()
        raise HTTPException(
...
@router.delete(
    "/{parecer_id}", 
    response_model=parecer_schemas.ParecerOut, # Ou um schema de confirmação
    summary="Deletar Parecer por ID (Admin + Sistema Autorizado)",
    description="Deleta um parecer pelo seu ID. Requer autenticação de usuário Admin E autenticação de sistema via X-API-KEY.",
    responses={
        status.HTTP_200_OK: {"description": "Parecer deletado com sucesso."},
        status.HTTP_401_UNAUTHORIZED: {"description": "Não autorizado."},
        status.HTTP_403_FORBIDDEN: {"description": "Proibido."},
        status.HTTP_404_NOT_FOUND: {"description": "Parecer não encontrado para deleção."},
        status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Erro interno do servidor."}
    }
)
async def delete_parecer_endpoint(
    parecer_id: int = Path(..., description="ID do parecer a ser deletado."), 
    db: AsyncSession = Depends(get_async_db),
    current_user: models.User = Depends(require_admin_user),
    authorized_system: SistemaAutorizado = Depends(get_current_authorized_system)
):
    try:
        db_parecer = await parecer_service.get_parecer_by_id(db, parecer_id)
        if db_parecer is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Parecer com ID {parecer_id} não encontrado para deleção.")
        
        deleted_parecer = await parecer_service.delete_parecer(db, db_parecer)
        return deleted_parecer
    except SQLAlchemyError as e:
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ocorreu um erro de banco de dados ao deletar o parecer {parecer_id}. Tente novamente. Erro: {str(e)}"
        )
    except HTTPException as e:
        raise e
    except Exception as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ocorreu um erro inesperado ao deletar o parecer {parecer_id}. Tente novamente. Erro: {str(e)}"
        )
